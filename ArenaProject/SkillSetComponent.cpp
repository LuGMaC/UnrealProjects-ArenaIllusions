...
USkillSetComponent::USkillSetComponent()
{
	...
	// ...
		...
		/*FString SkillPath = TEXT("C:/Users/Seph_/Documents/Unreal Projects/ArenaProject/ArenaProject/Content/Databases/SkillNinja.uasset");

		static ConstructorHelpers::FObjectFinder<UDataTable> SkillTableSearch(TEXT("C:/Users/Seph_/Documents/Unreal Projects/ArenaProject/ArenaProject/Content/Databases/SkillNinja.uasset"));

		if (SkillTableSearch.Succeeded())
		{
			UE_LOG(LogTemp, Warning, TEXT(" SkillTable manual pathway success"));
			SkillTable = SkillTableSearch.Object;
		}

		else
		{
			UE_LOG(LogTemp, Warning, TEXT(" SkillTable manual pathway failed"));
		}*/
...
void USkillSetComponent::BeginPlay()
{
	...
	if (SkillTable)
	{
		UE_LOG(LogTemp, Warning, TEXT(" SkillTable exists"));
	}

	else {

		UE_LOG(LogTemp, Warning, TEXT(" SkillTable does not exist"));
	}
...
// Called every frame
void USkillSetComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void USkillSetComponent::SetActiveSkill(FName RowName)
{
	static const FString ContextString(TEXT("ContextString"));
	FoundSkill = SkillTable->FindRow<FSkill>(RowName, ContextString);
...
void USkillSetComponent::SetActiveSkillByIndex(uint8 Index)
{
	ActiveSkill = ActivatedSkills[Index];
}

TArray<EAttackType> USkillSetComponent::CheckTurnSkills(TArray<EAttackType> TurnCommands)
{
	...
	*/
	...
	if (TurnCommands.Num() > 2)
	{
		...
		for (int i = 0; i < RowNames.Num(); i++)
		{
			...
			static const FString ContextString(TEXT("ContextString"));
			FoundSkill = SkillTable->FindRow<FSkill>(RowNames[i], ContextString);
...
void USkillSetComponent::SkillComparison(FSkill* SkillToCompare, TArray<EAttackType> Commands)
{
...
TArray<EAttackType> USkillSetComponent::SkillCommandsToArray(FSkill* Skill)
{
